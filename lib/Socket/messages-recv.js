...
export const makeMessagesRecvSocket = (config) => {
    ...
    const resolveMentionedPn = async (message) => {
        const content = normalizeMessageContent(message);
        const contentType = getContentType(content);
        const contextInfo = content?.[contentType]?.contextInfo;

        if (!contextInfo?.mentionedJid?.length) return;

        const lids = contextInfo.mentionedJid.filter(isLidUser);
        if (!lids.length) return;

        const mappings =
            await signalRepository.lidMapping.getPNsForLIDs(lids);

        contextInfo.mentionedPn = lids
            .map(lid => mappings.find(mapping => mapping.lid === lid)?.pn)
            .filter(Boolean)
            .map(pn => pn.replace(':0@', '@'));
    };
    ...
    const handleMessage = async (node) => {
        ...
        try {
            ...
            await messageMutex.mutex(async () => {
                ...
                await resolveMentionedPn(msg.message);

...
import { CALL_AUDIO_PREFIX, CALL_VIDEO_PREFIX, WA_AUTHOR_URL, CASILEYS_NAME, MEDIA_KEYS, URL_REGEX, WA_DEFAULT_EPHEMERAL } from '../Defaults/index.js';
...
const prepareStickerPackMessage = async (message, options) => {
    const { cover, stickers = [], name = '📦 Sticker Pack', publisher = 'GitHub: itsliaaa', description = '🏷️ Casileys' } = message;
...
const prepareNativeFlowButtons = (message) => {
    ...
    if (hasOptionalProperty(message, 'offerText') && !!message.offerText) {
        Object.assign(messageParamsJson, {
            limited_time_offer: {
                text: message.offerText || CASILEYS_NAME,
                url: message.offerUrl || WA_AUTHOR_URL, // Lia@Note 02-02-26 --- Apologies if this feels cheeky, just a fallback
...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasOptionalProperty(message, 'externalAdReply') && !!message.externalAdReply) {
        ...
        if (!content.url || typeof content.url !== 'string') {
            content.url = WA_AUTHOR_URL; // Lia@Note 02-02-26 --- Apologies if this feels cheeky, just a fallback
        }
        const externalAdReply = {
            ...
            title: content.title || CASILEYS_NAME
        };

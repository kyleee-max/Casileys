...
import { WA_AUTHOR_URL, LEXER_REGEX } from '../Defaults/index.js';
    ...
    sections: submessages.map((submessage, index) => {
        switch (submessage.messageType) {
            ...
            case RichSubMessageType.GRID_IMAGE: {
                const imageMetadata = submessage.gridImageMetadata;
                const image = imageMetadata?.imageUrls?.[0] || imageMetadata?.gridImageUrl;

                return {
                    view_model: {
                        primitive: {
                            media: {
                                url: image?.imagePreviewUrl || image?.imageHighResUrl || '',
                                mime_type: 'image/png'
                            },
                            imagine_type: 'IMAGE',
                            status: { status: 'READY' },
            ...
            case RichSubMessageType.INLINE_IMAGE: {
                const imageMetadata = submessage.imageMetadata;
                return {
                    view_model: {
                        primitive: {
                            media: {
                                url: imageMetadata?.imageUrl || imageMetadata?.imagePreviewUrl || '',
                                mime_type: 'image/png'
                            },
                            imagine_type: 'IMAGE',
                            status: { status: 'READY' },
...
export const prepareRichResponseMessage = (content) => {
    ...
    if (Array.isArray(richResponse)) {
        submessages = richResponse.map((submessage) => {
            ...
            else if (submessage.image) {
                const images = Array.isArray(submessage.image)
                    ? submessage.image
                    : [submessage.image];
        ...
        if (links) {
            links.forEach((linkField, index) => {
                ...
                const url = linkField.url || WA_AUTHOR_URL;
    ...
    console.log('[RICH DEBUG]', JSON.stringify(submessages, null, 2));
    const unified = toUnified(submessages, uuid);
    console.log('[UNIFIED DEBUG]', JSON.stringify(unified, null, 2));
